...
// Brand glow colors (RGB triplets, no rgba wrapper)
const CORAL = '217, 119, 87' // Claude coral
const GREEN = '0, 135, 55' // Technical Hub green
const SPOTLIGHT_COLOR = CORAL

// Flatten partners into a lookup, then place them into the bento layout.
const byId = Object.fromEntries(
  partnerTiers.flatMap((t) => t.partners.map((p) => [p.id, { ...p, accent: t.accent, tierLabel: t.label }]))
)
...
const LAYOUT = [
  ...
  const Icon = p.accent === 'coral' ? Spark : Academic
  return { ...slot, ...p, logo: LOGOS[slot.id] || p.logo, glow, Icon }
})
...
const GlobalSpotlight = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = SPOTLIGHT_COLOR }) => {
...
export default function MagicBento({
  ...
  // stagger the cards in each time the grid enters view
  useEffect(() => {
  ...
  return (
    <>
      <GlobalSpotlight gridRef={gridRef} disableAnimations={shouldDisable} spotlightRadius={spotlightRadius} />
      <div className={`card-grid bento-section ${inView ? 'in-view' : ''}`} ref={gridRef}>
        {LAYOUT.map((p, i) => (
          <ParticleCard
            key={p.id}
            className={`magic-bento-card magic-bento-card--border-glow bento-${p.area} ${p.big ? 'magic-bento-card--big' : ''}`}
            style={{ '--glow-color': p.glow, animationDelay: `${i * 110}ms` }}
        ...
        {/* ASSOCIATED WITH heading — bottom-right cell */}
        <div className="bento-text">
          <span className="bento-text__eyebrow">Trusted by</span>
          <ScrollFloat containerClassName="bento-text__title" stagger={0.04}>
            Associated
          </ScrollFloat>
          <ScrollFloat containerClassName="bento-text__title" stagger={0.04}>
            With
